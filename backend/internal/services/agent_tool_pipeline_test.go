...
func TestResolveToolInvocation_SearchMemory(t *testing.T) {
	...
	if invocation.toolName != consts.SearchMemoryTool {
		t.Fatalf("unexpected toolName: %s", invocation.toolName)
	}
...
func TestExecuteInvocation_SearchMemory_OncePerResponse(t *testing.T) {
	...
	first := agent.executeInvocation(
		context.Background(),
		ToolCallInfo{ID: "call_3", Name: consts.SearchMemoryTool},
		resolvedToolInvocation{toolName: consts.SearchMemoryTool, command: "query"},
	...
	second := agent.executeInvocation(
		context.Background(),
		ToolCallInfo{ID: "call_4", Name: consts.SearchMemoryTool},
		resolvedToolInvocation{toolName: consts.SearchMemoryTool, command: "query"},
		map[string]string{"query": "golang"},
		"current-session",
		nil,
