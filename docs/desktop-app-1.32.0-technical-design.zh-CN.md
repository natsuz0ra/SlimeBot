...
`desktop/` 管理 Electron 主进程、preload、图标与安装配置；Go 入口添加 `desktop-host` 子命令。`desktop-host` 运行完整业务服务，包括调度器和已配置的 Telegram。数据位于 Electron 的平台 `userData/data`（macOS 通常是 `~/Library/Application Support/slimebot-desktop/data`，Windows 通常是 `%APPDATA%/slimebot-desktop/data`），与脚本安装的 `~/.slimebot` 隔离。桌面入口强制将数据库、Skills 和上传路径指向该目录，即使旧配置模板写有 Web 默认路径也不会复用 Web 数据。用户若在两处配置同一个 Telegram 机器人，须自行停用其中一处以免重复消费消息。
...
2. 主进程确定专属数据目录，启动打包的 Go 可执行文件。启动随机密钥经 stdin 管道传递，主进程覆盖 `SLIMEBOT_HOME` 并清除会导致共用 Web 数据或密钥的环境变量；工具执行继续走现有审批与沙盒。
...
桌面采用用户指定的本机单用户免登录语义：不种子 `admin/admin`，不展示登录、改密或登出入口，Go 不注册登录/改密 API。脚本安装的 Web 服务继续使用账号密码。桌面与服务使用独立数据库；JWT 密钥仅在本次桌面进程内存中设置，不进入命令行参数或 URL。
...
数据、配置、Skills 和上传统一位于桌面数据目录；Electron 自己的缓存位于平台 `userData`。首版不做旧版 Web/CLI 数据自动迁移，避免静默共用数据库或覆盖配置。
...
| 阶段 | 工作 | 可验收结果 |
| --- | --- | --- |
| A. 安全启动基础 | Go `desktop-host`、随机端口/回环监听、ready 管道、独立数据目录、无密码本机会话 | 双击启动；局域网无法访问；同机其他进程无法通过猜端口取得 Agent 权限 |
