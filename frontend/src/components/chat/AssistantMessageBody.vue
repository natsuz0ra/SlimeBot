  ...
  <div class="text-sm leading-relaxed w-full">
    <div v-if="getReplyToolCount(item.id) > 0" class="assistant-tool-summary-row mb-2.5">
      <button
        ...
        :aria-label="`${toolExecutionDetailTitle} - ${getReplyToolSummary(item.id)}`"
      ...
      <template v-for="entry in getReplyTimeline(item.id)" :key="entry.id">
        ...
        >
          <ToolCallCard
            ...
            :show-preamble="false"
            @approve="approveToolCall($event, true)"
            @reject="approveToolCall($event, false)"
          />
        </div>
      </template>
    </div>
    ...
    <TypingDots v-if="isEmptyPlaceholder(item.id) && waiting" />
  </div>
</template>
