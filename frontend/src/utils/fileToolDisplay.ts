...
export function buildFileToolDisplays(item: {
  toolName?: string
  ...
  const metadataDisplays = displaysFromMetadata(toolName, item.metadata)
  ...
  return [{
    ...
    fileName: baseName(filePath),
    operation: 'Write',
    summary: filePath ? `Wrote ${lineCount} ${lineCount === 1 ? 'line' : 'lines'} to ${baseName(filePath)}` : buildBatchSummary(toolName, params),
...
export function buildFileToolDisplay(item: {
  ...
  return buildFileToolDisplays(item)[0] || null
}
