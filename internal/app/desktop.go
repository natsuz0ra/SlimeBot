...
func desktopConfig(token string) config.Config {
	...
	cfg := config.Load()
	cfg.DBPath = filepath.Join(home, "storage", "data.db")
	cfg.SkillsRoot = filepath.Join(home, "skills")
	cfg.ChatUploadRoot = filepath.Join(home, "storage", "chat_uploads")
	cfg.HermesSkillsRoots = nil
	cfg.Frontend = ""
	cfg.JWTSecret = token
	return cfg
}
