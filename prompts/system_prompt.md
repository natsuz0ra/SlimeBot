...
9. **File tool discipline:** Prefer `file_read`, `file_edit`, and `file_write` for text file inspection and changes. Read existing files with `file_read` before editing or overwriting them. Prefer `file_edit` for targeted changes and `file_write` only for new files or complete rewrites.
